...
| 复杂实现 / 跨文件 | 10% | 调度层（TwoOne）|
| 困难 bug | 3% | 调度层（TwoOne + diagnose）|
...
| 你想做的事 | 用什么 | 走哪层 |
|-----------|--------|--------|
| 写一行 bug fix | 直接改 | 调度层（OneTwo self）|
| 写 5 个 CRUD 文件 | 委派 EggDog | 调度层 |
...
| 风险 | 缓解 |
|------|------|
| LLM 误触发 `/opsx:propose` 因为 prompt 里有 OpenSpec 关键词 | OneTwo 必须**明确说** propose/apply/sync/archive（Layer 1 强触发）才走 |
...
1. User: "提议一个新功能 X"
   ...
   → 委派 TwoOne 子 agent 分析 tasks.md 的可行性
...
OneTwo 不应自己直接调 openspec CLI 写文件；它应：
1. Load this skill
2. 用自然语言告诉 TwoOne "读 openspec/changes/X/proposal.md 并分析可行性"
3. TwoOne 返回 `<results>` 块
4. OneTwo 决策后，用 `write` 工具写新文件或用 `hashline-edit` 改现有文件
