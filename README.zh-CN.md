...
> 我们项目自带 **19 个通用 skill**（流程/工作流导向，见 `VIBECODING.md` 看操作员手册）。**领域专用 skill**（框架、数据库、语言）我们提供**精选推荐目录**——你用 `npx skills` CLI 自己装。
>
...
> 3. 内置 `task` 工具和 OneTwo prompt 教的协议对齐——见 `agents/sisyphus.md` 的 `<delegation_protocol>` 段
...
└───┬────┘└───┬────┘└────────┘└────────┘└────┬────┘└─────────┘
    │         │                                 │
    │(architect→update) (planner→lyra→hepheast.)│
    └─write ADR─┘                                │
                                                  ▼
                                       (TwoOne → EggDog)
```
...
| `task` 委派 | lyra/hephaestus/architect/planner/reviewer/update | hephaestus | ❌ deny（叶子）| update/lyra | architect/update/hephaestus | ❌ deny（独立 review）| ❌ deny（单写者）|
...
| 组件 | 类型 | 数量 | 来源 |
|------|------|------|------|
| Agents | `.md` prompt 文件 | **7** | 自研（sisyphus/lyra/hephaestus + update/architect/planner/reviewer） |
...
│   ├── lyra.md             # subagent (mid-tier) — 可委派给 EggDog
