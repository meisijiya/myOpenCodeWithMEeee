...
> *"这个 agent 团队体系，是我为我和女朋友打造的。"*
>
> 当我和女朋友开始一起做项目时，我们意识到需要一个能处理不同类型任务的编码助手——从高层架构决策到重复的 CRUD 工作。我们希望它像一个**家庭**：一个理解我们需求的管家，一个处理难题的技术专家，还有精力充沛的帮手来做琐碎工作。
>
> 所以我们创造了 **OneTwo**（管家）、**TwoOne**（赚钱的）和 **EggDog**（精力充沛的小孩）——加上一组专家（update/architect/planner/reviewer）来处理特定情况。这不仅仅是一个工具，它是我们的小小编码家庭，帮助我们共同构建未来。🏠💻

---

## 🛒 找领域专用 Skill？（React / Java / Docker 等）

> 我们项目自带 **19 个通用 skill**（流程/工作流导向，见 `VIBECODING.md` 看操作员手册）。**领域专用 skill**（框架、数据库、语言）可以用 `npx skills` CLI 自己装。
>
> 🔍 **先去市场找**: [skillsmp.com/zh](https://skillsmp.com/zh) · [ai.codefather.cn/skills](https://ai.codefather.cn/skills) · [vercel-labs/skills](https://github.com/vercel-labs/skills)
>
> ⚠️ **装前必读**：领域 skill 安装前注意重名冲突、token 预算、权限模型。
...
Superpowers 提供 14 个流程编排 skill（brainstorming → writing-plans → subagent-driven-dev → review → finish），OneTwo 的 `<openspec_protocol>` 路由依赖它做默认工作流。
...
> 如果不装 Superpowers，OneTwo 会跳过这些 skill，用 opencode 内置工具退化工作。
...
我们的 `caveman`、`diagnose`、`grill-with-docs`、`handoff`、`improve-codebase-architecture`、`prototype`、`setup-matt-pocock-skills`、`tdd`、`to-issues`、`triage`、`zoom-out` 是从 [mattpocock/skills](https://github.com/mattpocock/skills) **原样导入**的（verbatim import）——只取了项目实际需要的 11 个，不是全量。**不要**另外跑 mattpocock 的官方 setup 脚本，否则同名 skill 会冲突，opencode 按文件系统遍历顺序去重，结果不可预测。

| 方式 | 效果 |
|------|------|
| ✅ 用我们的 `install.sh` | 恰好 11 个，无冲突 |
| ❌ 额外跑 mattpocock 官方 setup | 11 个重名 skill，不确定哪个生效 |
...
| 组件 | 原因 | 安装 |
|------|------|------|
| **Superpowers (14 skills)** | 提供 OneTwo 主用的默认工作流（brainstorming → writing-plans → subagent-driven-dev → review → finish） | `opencode plugin install superpowers` |
| **RTK (Rust Token Killer)** | 常用命令省 60-90% token（`git status`/`ls`/`grep`/`cargo test`/...） | `brew install rtk && rtk init -g --opencode` |

缺这些时 OneTwo 仍能用，但：
- 没 Superpowers → OneTwo `<openspec_protocol>` 默认回退到 opencode 内置工具，工作流没那么结构化
...
### 本项目自带的 19 个 Skill（`bash install.sh` 自动装）

| Skill | 类型 | 触发 |
|-------|------|------|
| `karpathy-guidelines` | 自研（4 大 karpathy 原则） | 通过 meta-skill prompt 注入自动加载每次 LLM 调用 |
| `openspec-integration` | 自研（OpenSpec ↔ Superpowers 路由桥） | 双层触发：关键词 OR 语义意图（多步/跨 spec） |
| `update-project-meta` | 自研（CONTEXT.md / AGENTS.md / ADR 维护）| update agent 唯一写者；用户调 `/updateProjectMeta` |
...
| `triage` | 从 mattpocock/skills 导入 | 5 角色 issue 状态机 |
| `improve-codebase-architecture` | 从 mattpocock/skills 导入 | Architect 主用——发现架构改进点 |
| `setup-matt-pocock-skills` | 从 mattpocock/skills 导入 | 一次性脚手架（issue tracker + triage 标签）|
...
安装完后，编辑 `~/.config/opencode/opencode.json`，添加 7 个 agent 的 model 配置：
...
> 注：`install.sh` 会自动注册 7 个 agent（含 prompt + tool 配置），你只需要填**每个 agent 的 model ID**。

### 推荐搭配

| 档位 | 角色 | Agent | 推荐模型（示例） | 月费估算 |
|------|------|-------|-----------------|---------|
| **high** | 架构 / 规划 / 评审 / 元信息写入 | OneTwo · Architect · Planner · Reviewer · Update | `anthropic/claude-opus-4-20250514` 或 `deepseek/deepseek-v4-pro` | ~$200 |
| **mid** | 复杂实现、调研、中等难度 bug | TwoOne | `anthropic/claude-sonnet-4-20250514` 或 `deepseek/deepseek-v4-flash` | ~$20 |
| **low** | CRUD、机械重构、测试 boilerplate | EggDog | `anthropic/claude-haiku-4-20250514` 或 `deepseek/deepseek-v4-flash-free` | ~$0 |

> 完整委派地图和档位理由见[架构](#架构)。

### 全局默认模型（省事方案）

如果不想配 7 个 agent，可以只设全局 model：
...
### 推荐使用 OneTwo 而非 build

安装后，opencode 默认提供两个 primary agent：**build** 和 **plan**。装了我们项目后，**OneTwo**（也是 primary）变为可用入口，6 个 subagent（TwoOne / EggDog / Architect / Planner / Reviewer / Update）通过 `@<agent>` 或 `task()` 触达。

**首次启动 opencode 时**（按 **Tab** 循环切换 primary agent）：

```
┌─ build (opencode 原生)         ← 全功能但无 7-agent 路由
...
| Agent | 何时用 |
|-------|--------|
| **OneTwo** | 99% 任务——架构、委派、跨文件、复杂实现 |
| **build** | 只想跑单个 shell 命令/快速 read 文件，不需要 OneTwo 的 7-agent 路由 |
| **plan** | 只做只读分析，零修改 |

**快捷切换**：在 TUI 里按 **Tab**（或 `switch_agent` keybind）循环 OneTwo 和 build。

**@ 委派子 agent**：
```
@twoone 帮我实现这个 feature
@eggdog 创建 5 个 CRUD 文件
@architect 设计新子系统的模块边界
@planner 写实现计划 + issue 拆解
@reviewer merge 前审计这个 PR
@update 把这个决策加到 docs/adr/
```

也可以在对话里直接让 OneTwo 委派（按它的 12 行 intent_gate 路由表自动选择——见下方[架构](#架构)）。

---

## 🚀 异步委派（opencode 1.16.2+）

v2.2 退役了 v2.1 的自研异步委派工具——直接用 **opencode 内置 `task` 工具**。opencode 1.16.2+ 原生支持后台子 agent，不需要自定义 wrapper。

### 基本模式

OneTwo / TwoOne 用 `task` 工具委派给 subagent。需要环境变量 `OPENCODE_EXPERIMENTAL=true`（或 `..._BACKGROUND_SUBAGENTS=true`）。

| 行为 | opencode 内置方式 |
|------|-------------------|
| **Fire-and-forget**（默认异步）| `task` 工具 + `run_in_background: true` |
| **阻塞等结果**| `task` 工具（同步）|
| **续接之前 session**（保留对话历史）| `task` 工具 + `session_id` 参数 |

**Fan-out**（多个子 agent 真并行）：在同一个 response 里循环调 N 次 `task` 工具，每次 `run_in_background: true`——opencode 让它们真正并行启动。

> 💡 **为什么 v2.2 不再包一层异步委派工具？**
> 1. opencode 1.17+ 已经支持后台子 agent + session 续接 + fan-out——v2.1 自研工具的差异化优势**不再独特**
> 2. 自研 wrapper 增加维护成本（一层 TypeScript 工具 = 一处可能 drift 的代码）
> 3. 内置 `task` 工具和 OneTwo prompt 教的协议对齐——见 `agents/onetwo.md` 的 `<delegation_protocol>` 段
...
决策树见 `agents/onetwo.md` 和 `agents/twoone.md` 的 `<delegation_protocol>` 段。
...
| **[Matt Pocock skills](https://github.com/mattpocock/skills)** | 11 个 verbatim 导入：caveman / diagnose / grill-with-docs / handoff / improve-codebase-architecture / prototype / setup-matt-pocock-skills / tdd / to-issues / triage / zoom-out | 不跑官方 installer（同名 skill 冲突风险；我们挑了需要的 11 个） |
...
| **[oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent)** | 架构灵感（主调度 → 多专家）| 不引入 100K+ LOC 代码库。opencode 1.16.2+ 原生支持后台子 agent（无需 HTTP 代理）|
| **[oh-my-opencode-slim](https://github.com/alvinunreal/oh-my-opencode-slim)** | 角色权限分层（只读 vs 读写）| 不引入 5+ 专家 agent（保持 7 个 / 3 档）|
| **[addyosmani/agent-skills](https://github.com/addyosmani/agent-skills)** | `interview-me`（原样） + `source-driven-development`（轻量复刻） + `git-workflow-and-versioning` + `incremental-implementation` | 不导入全部 23 个 skill（轻量原则）|
...
| 洞察 | 应用 |
|------|------|
| **U 型注意力曲线**（整个 context 窗口 → 首+末都高注意力）| **修正**：U 型描述的是**整个 LLM context 窗口**（system prompt + 对话 + 工具结果），不是 agent 提示词内部结构。**System prompt 区域** = "首"高注意力区（含 7 个 agent 提示词）。**最近对话** = "末"高注意力区。**Context 中段** = 低注意力（早期对话被挤到中段）。HTML 末尾铁律在"首"区（稳定），不在"末"区 |
| **硬约束词汇**（never/always/must/绝对不要）| 重写 7 个 agent 的 `style_guide` 用强约束 + 反例/正例；末尾 HTML 块用 `**bold**` 加粗 + 括号内解释理由 |
| **Skill 文件 ≤ 300-500 行**| 全部 skill 都在 250 行内；**OneTwo.md 现在 253 行**（从 363 行减下来——148 行硬编码协议被下沉到 superpowers skill 实时注入）|
| **方案 1+2+3+4**（AGENTS.md + Scan + Hooks + 子 Agent 隔离）| 全部有：`karpathy-guidelines` 通过 meta-skill prompt 注入 = Hook；子 agent 隔离 = 7-agent 架构核心；**职责就近原则**（每个 agent 在自己 prompt 里用 `<responsibility_boundary>` 块声明——不依赖外部 AGENTS.md）|
| **反被动压缩**（不等质量下降再压缩）| **真实失败案例**：OneTwo 363 行 prompt 变得难维护。v2.1 用了自研异步委派工具而非 opencode 内置 `task`——**不是因为"被 U 型注意力吞掉"**（协议在中段块中**是可见的**），而是**分层违规**（自研 wrapper 增加了维护成本 + 跟 opencode 升级 drift）。修复：v2.2 退役自研 wrapper，回到 opencode 内置 `task`——简化架构 > 短期"看起来更优雅"的自研 |
...
| **机制层 vs 装饰层分层**（新）| OneTwo prompt = **Static 层**（路由判断 / 风格 / 硬约束）+ **Dynamic 层**（协议由 superpowers skill 注入）。Static 层短而高信号，活在尾部 |
| **6 块结构对称**（新）| 7 个 agent prompt 共享结构：`role` / `responsibility_boundary` / `capabilities` / `style_guide` + 末尾 HTML 块。差异在**内容**（业务能力、工具列表、风格），不在**结构**。可用 grep 跨 agent 审计 |
...
│  • Routes to 6 sub-agents via 12-row intent_gate         │
...
└──┬──────┬──────┬──────┬──────┬────────────────────────────┘
   ...
   ▼      ▼      ▼      ▼      ▼      ▼
┌────────┐┌────────┐┌────────┐┌────────┐┌─────────┐┌─────────┐
│Architect││Planner ││Reviewer││Update  ││TwoOne     ││Hephaest.│
│(high)  ││(high)  ││(high)  ││(high)  ││(mid)    ││(low)    │
│Domain  ││Plans + ││Code    ││CONTEXT ││Complex  ││CRUD /   │
│model + ││to-issues│ review ││  .md/  ││  impl + ││refactor/│
│ADR     ││        ││  + verif││ADR/    ││research ││test     │
│drafts  ││        ││        ││AGENTS  ││         ││boilerpl.│
└───┬────┘└───┬────┘└────────┘└────────┘└────┬────┘└─────────┘
    │         │                                 │
    │(architect→update) (planner→twoone→hepheast.)│
    └─write ADR─┘                                │
                                                  ▼
                                       (TwoOne → EggDog)
```

**Strict depth=3 rule**: EggDog + Update + Architect + Planner + Reviewer 全部 `task: false`（opencode 硬保证——Task 工具**被物理移除**）。TwoOne + OneTwo 可以委派子 agent，但 TwoOne **禁止直接 spawn EggDog**——必须经过 OneTwo intent_gate。无无限递归。

### Agent 角色速览

| Agent | Tier | 能做 | 不做 |
|-------|------|------|------|
| **OneTwo** | high | 意图路由、OpenSpec 编排、skill 发现 | 实现、code review、元信息写入 |
| **Architect** | high | 领域建模、模块边界、ADR 草案 | code review、写代码、写 plan |
| **Planner** | high | 实现计划、issue 拆解 | 架构设计、code review、写代码 |
| **Reviewer** | high | 独立 code review、完成前验证 | 写代码、规划、架构 |
| **Update** | high | CONTEXT.md / ADR / AGENTS.md 唯一写者 | 实现、规划、review |
| **TwoOne** | mid | 复杂实现、调研、中等 bug | 架构设计、code review、写 plan |
| **EggDog** | low | CRUD、原子重构、测试脚手架 | 架构、规划、review、元信息写入 |

### 路由逻辑（12 行 intent_gate）

| Intent | 触发 | 路由 | Tier | OpenSpec |
|--------|------|------|------|----------|
| ARCHITECTURE | 重大架构决策 / 模块边界 / 领域建模 | **Architect** | high | yes |
| DESIGN | 新 feature 设计（含单文件 + 设计内容）| OneTwo（self） | high | yes |
| COMPLEX_CODE | 跨文件新 feature（≥2 文件需协调）| **TwoOne** | mid | yes |
| RESEARCH | 调研、文档 | **TwoOne** | mid | no |
| DEBUG_HARD | 复杂 bug（diagnose + fix + test verify）| **TwoOne** | mid | no |
| DEBUG_SIMPLE | 明显 bug（单文件 ≤10 行）| OneTwo（self） | high | no |
| CRUD | 重复写入（3+ 相似文件）| **EggDog** | low | no |
| ATOMIC_REFACTOR | 机械转换（跨文件重命名）| **EggDog** | low | no |
| TEST_BOILERPLATE | 测试脚手架 | **EggDog** | low | no |
| PLAN | 实现计划 / issue 拆解 | **Planner** | high | no |
| REVIEW | code review / 完成前验证 / 接收反馈 | **Reviewer** | high | no |
| META_WRITE | 写 CONTEXT.md / ADR / AGENTS.md（术语 / 约定）| **Update** | high | no |

**核心判断**：**推理复杂度**（不是文件数）决定档位。

**边界情况**（避免"我自己干"的冲动）：
- "改 1 文件 + 跑 quick command" → DEBUG_SIMPLE / OneTwo（self）
- "改 1 文件 + 跑完整 test suite" → DEBUG_HARD / TwoOne（验证**就是**调研）
- "写 1 个设计文档（CONTRIBUTING.md）" → DESIGN / OneTwo（self）
- "写 README + install.sh 联动" → COMPLEX_CODE / TwoOne（多文件协调）

> 完整委派流程图和决策树见 `VIBECODING.md`。
...
|-------|------|-----------|-----------|----------------------|
| **OneTwo** | high | 主 agent（承载完整会话）| 架构决策 + 复杂推理 + **单文件 ≤10 行的小改** + 意图路由 | 跨多文件改动 / 复杂调研 / 简单 CRUD / 元信息写入 |
| **Architect** | high | 纯净 subagent | 领域建模 + 模块边界 + ADR 草案 | 写代码 / code review / 写 plan |
| **Planner** | high | 纯净 subagent | 实现计划 + issue 拆解（`to-issues`）| 架构设计 / 写代码 / code review |
| **Reviewer** | high | 纯净 subagent（只读）| 独立 code review + 完成前验证 | 写代码 / 规划 / 架构 |
| **Update** | high | 纯净 subagent（唯一写者）| CONTEXT.md / ADR / AGENTS.md 元信息维护 | 实现 / 规划 / review |
| **TwoOne** | mid | 纯净 subagent（不继承主会话）| 复杂调研 + 多文件实现 + 中等 bug + 结构化输出 | 架构决策 / 简单 CRUD / 简单搜集资料 |
| **EggDog** | low | 纯净 subagent 叶子（不可再委派）| 简单搜集（mmx search CLI）+ CRUD 脚手架 + 原子重构 + 测试 boilerplate | 复杂调研 / 架构决策 / 委派子 agent |

**判定规则**（明确边界，避免 Agent 抢活）：
- OneTwo 写代码仅限"**当前 turn 用户直接要求 + ≤1 文件 + ≤10 行**"（与 intent_gate 的 DEBUG_SIMPLE 对齐）
- 简单搜集资料（mmx search）→ **EggDog**（不是 TwoOne，也不是 OneTwo）
- 复杂调研（多步 + 需要分析）→ **TwoOne**（不是 OneTwo，也不是 EggDog）
- 跨多文件改动 → **TwoOne**（多文件设计）或 **EggDog**（机械批量）
- 架构决策 → **Architect**（不是 OneTwo）
- 写 plan / issue 拆解 → **Planner**
- code review → **Reviewer**
- 写 CONTEXT.md / ADR / AGENTS.md → **Update**（唯一写者）

**U 型注意力对齐**：7 个 agent 都遵循 6 块对称结构（`role` / `responsibility_boundary` / `capabilities` / `style_guide` + 末尾 HTML 铁律）——差异仅在**内容**（业务能力、工具列表、风格细节），不在**结构**。
...
| 操作 | OneTwo | TwoOne | EggDog | Architect | Planner | Reviewer | Update |
|------|:--------:|:----:|:----------:|:---------:|:-------:|:--------:|:------:|
| `read / grep / glob / webfetch / websearch` | ✅ allow | ✅ allow | ✅ allow（websearch deny） | ✅ allow | ✅ allow | ✅ allow | ✅ allow |
| `edit / write` 项目内 | ✅ allow | ✅ allow | ✅ allow | ✅ allow（仅设计文档）| ✅ allow（仅 plans）| ❌ deny（只读）| ✅ allow（仅元信息文件）|
| `edit / write` 项目外（`external_directory`） | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask |
| `edit / write` `.env*` | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny |
| `bash` 项目内（默认） | ✅ **allow** | ✅ **allow** | ✅ allow | ✅ allow | ⚠️ ask | ⚠️ ask | ⚠️ ask |
| `bash` 危险（`rm -rf /`、`sudo`、`mkfs`、`dd`、`chmod -R 777`）| ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny |
| `bash` 强制推送/硬重置（`git push --force`/`reset --hard`/`clean -fd`）| ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny |
| `bash` 包发布（`npm/pnpm/yarn/cargo publish`、`twine upload`）| ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny | ❌ deny |
| `task` 委派 | twoone/eggdog/architect/planner/reviewer/update | eggdog | ❌ deny（叶子）| update/twoone | architect/update/eggdog | ❌ deny（独立 review）| ❌ deny（单写者）|
| `external_directory`（项目外访问）| ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask |
| `doom_loop`（3 次相同调用）| ⚠️ ask（默认）| ⚠️ ask（默认）| ⚠️ ask（默认）| ⚠️ ask | ⚠️ ask | ⚠️ ask | ⚠️ ask |

### 相对 v1 改了什么

| 维度 | v1（旧）| v2（现）|
|------|---------|---------|
| `bash` 默认 OneTwo/TwoOne | `ask`（太谨慎）| **`allow`**（项目信任）|
| 项目外 `edit`/`write` | 自定义 `**/../**` glob（漏报）| `external_directory: ask`（opencode 原生）|
| 安全机制 | 多层 ask 提示 | 切到 `build`/`plan` agent |

### 设计意图（omo-slim 启发）

- **角色权限分层**：omo-slim 按角色给不同 agent 不同默认信任——只读 agent（`explorer`/`librarian`/`oracle`/`observer`）vs 读写 agent（`designer`/`fixer`）。我们也这样：EggDog 最宽松（worker 需要大量命令），OneTwo/TwoOne 中等（默认 allow + 硬 deny 黑名单），`build`/`plan` 最保守（opencode 出厂默认）。新加的 Architect/Planner/Reviewer/Update 按职责细化（Architect 仅写设计文档、Reviewer 只读、Update 仅写元信息文件、Planner 需要用户确认 bash）。
...
|-------|------|-----------|-----------|
| **karpathy-guidelines** | multica-ai/andrej-karpathy-skills | 所有 LLM 调用（meta-skill prompt 注入自动加载） | 全部 |
| **grill-with-docs** | mattpocock/skills | 压测计划与领域模型一致性 | OneTwo / TwoOne |
| **caveman** | mattpocock/skills | 超压缩通信（-75% tokens） | OneTwo / TwoOne |
| **diagnose** | mattpocock/skills | 硬 bug、性能回归（6 阶段循环） | TwoOne（主场景） |
| **prototype** | mattpocock/skills | 一次性原型验证设计 | OneTwo / TwoOne |
| **to-issues** | mattpocock/skills | 把 plan/spec 拆成独立 issues | Planner |
| **triage** | mattpocock/skills | 5 角色 issue 状态机 | OneTwo / Planner |
| **improve-codebase-architecture** | mattpocock/skills | 发现"代码泥球"+ 建议架构改进 | Architect |
| **setup-matt-pocock-skills** | mattpocock/skills | 一次性脚手架（issue tracker + triage 标签）| OneTwo（一次性）|
| **git-workflow-and-versioning** | addyosmani/agent-skills（原样导入）| Git 工作流：原子提交、分支策略、冲突解决 | 全部 |
| **incremental-implementation** | addyosmani/agent-skills（原样导入）| 纵向切片实现，补充 tdd | TwoOne / Planner |
| **interview-me** | addyosmani/agent-skills（原样导入）| 需求不明确（缺 who/why/success/constraint）—— 一次问一个问题直到 95% 置信度 | OneTwo / Planner |
| **source-driven-development** | addyosmani/agent-skills（轻量复刻）| 框架/API 决策需要官方文档验证（用 ctx7 CLI）| TwoOne（实现时）|
| **handoff** | mattpocock/skills（原样导入）| 将当前对话压缩为交接文档给下一个 agent | 全部（用户触发）|
| **tdd** | mattpocock/skills（原样导入）| 红-绿-重构循环的测试驱动开发 | TwoOne / EggDog |
| **zoom-out** | mattpocock/skills（原样导入）| 放大视野，获取更广上下文和高层视角 | 全部（用户触发）|
| **update-project-meta** | 自研 | CONTEXT.md / ADR / AGENTS.md 元信息维护 | Update（单写者）/ OneTwo |
...
**何时不要用**（反模式）:
- ❌ **子 agent 委派** — 那是 OneTwo → TwoOne → EggDog，用 `<delegation_protocol>`（不是 handoff）
...
| 7-agent 集成 | ❌ 无（仅用户）| ⚠️ 跨 agent 可用，但不自动 |
...
| Command | 功能 | 谁用 |
|---------|------|------|
| `/opsx:propose` | 创建 change 提案 | OneTwo / TwoOne |
| `/opsx:explore` | 自由探索 | OneTwo / TwoOne |
| `/opsx:apply` | 实施 tasks.md | TwoOne（委派执行） |
| `/opsx:sync` | 合并 delta → 主 spec | OneTwo |
| `/opsx:archive` | 归档完成的 change | OneTwo |

**EggDog 全部绕过 OpenSpec**——CRUD 不需要规约。
...
```
┌──────────────────────────────────────────────────────────────┐
│ 第1层：调度（我们的 7 个 agent）                              │
│  OneTwo / TwoOne / EggDog / Architect / Planner /         │
│  Reviewer / Update — 意图路由 + 委派                          │
│  "谁做什么？"                                                 │
├──────────────────────────────────────────────────────────────┤
│ 第2层：工作流（我们的 19 个 skill + Superpowers）              │
...
| 跨文件实现 | 10% | 第 1 层（TwoOne / Architect / Planner）|
| 困难 bug | 3% | 第 1 层（TwoOne + diagnose）|
...
**SUGGEST 模板**（OneTwo 在 Layer 2 匹配时说）：
...
本项目的**全量工作流底座**。Superpowers 提供从创意到合并的完整流程编排（brainstorming → writing-plans → subagent-driven-dev → review → finish），OneTwo 的 `<openspec_protocol>` 段负责路由：提到 OpenSpec 关键词 → 走 OpenSpec；默认 → 走 Superpowers。
...
这些是**领域知识型** skill，与我们 19 个**流程型** skill（karpathy、diagnose、tdd 等）互补，无重叠。
...
| Plugins | 运行时 hook | **0** | （v2.2 退役；karpathy-guidelines 通过 meta-skill prompt 注入自动加载） |
| CLIs | npm -g | **3** | mmx-cli (MiniMax 多模态+搜索) + ctx7 (库文档) + playwright-cli (浏览器自动化) |

> 21 个 command 是**用户入口**——完整目录见 [`VIBECODING.md`](./VIBECODING.md)。
...
```bash
vim agents/onetwo.md   # 改路由规则、委派协议、style_guide
...
- Command：删掉 `.opencode/commands/<name>.md`
...
## Skill 路由（项目级）
本项目用 OneTwo（不是 build）。默认工作流：
- 启动需求 → Superpowers `brainstorming`
- 写代码 → `twoone` subagent（mid-tier）
- 重复性任务 → `eggdog` subagent（low-tier）
...
#### 4. 项目级 Agent 覆盖
如果某个项目想覆盖我们 7 个 agent 中某一个的行为，在 `.opencode/agents/` 放同名 `.md`：

```bash
mkdir -p .opencode/agents
cp ~/.config/opencode/agents/onetwo.md .opencode/agents/onetwo.md
# 编辑项目级 onetwo.md（修改 intent_gate 路由等）
```
...
| 模型 | 上下文 | 触发点 | 说明 |
|------|-------|--------|------|
| **MiniMax-M3**（high：OneTwo / Architect / Planner / Reviewer / Update）| 512K | **~422K** | ✅ 512K 模型优化（reserved 50K + preserve 50K）|
| **deepseek-v4-flash**（mid/low：TwoOne / EggDog）| 1M | ~910K | 激进 prompt 让 session 远离 910K |

OneTwo 主 agent **372K 触发 ≈ 340K 目标**——你指定的甜点。
...
> **为什么**：Session 是短暂的，但项目知识应该持久。我们的项目记忆系统在 session 之间保存关键决策、术语和约定。

### 工作原理

| 文件 | 用途 | 何时更新 |
|------|------|---------|
| `CONTEXT.md` | 核心术语和概念 | 发现新术语时 |
| `AGENTS.md` | 项目约定和规范 | 建立新约定时 |
| `docs/adr/NNNN-xxx.md` | 架构决策记录（ADR）| 做出架构决策时 |

### 工作流

**1. 首次初始化**（一次性）：
```bash
/setup  # 初始化 CONTEXT.md / AGENTS.md / docs/adr/
```

**2. 开发过程中**：
```bash
/updateProjectMeta  # 记录新术语/决策/约定
```

**3. 关闭 session 前**（推荐）：
```bash
/updateProjectMeta  # 保存本次 session 的关键信息
```

**4. 下次 session 启动时**：
- OneTwo 自动读取 `CONTEXT.md` / `AGENTS.md` / `docs/adr/`
- 项目上下文自动恢复——无需重新解释

### 最佳实践

> 💡 **关闭 session 前**，跑 `/updateProjectMeta` 保存关键信息。这确保下次 session 能从你离开的地方继续。
...
> v2.2 起没有 TypeScript 工具需要构建——所有自研工具已由 CLI 调用和 opencode 内置工具替代。
```
...
> **我们 19 个全局 skill 中有 15 个来自上游**（mattpocock/skills, addyosmani/agent-skills）。每个来源都在 [`skills/SOURCES.yaml`](skills/SOURCES.yaml) 留档，并附带检查+更新脚本。

### Skill 来源汇总

| 类型 | 数量 | 更新策略 |
|------|------|---------|
| **self**（无上游）| 4 个（karpathy, openspec-integration, mmx-cli-usage, update-project-meta）| N/A |
| **verbatim**（100% 原样）| 14 个（mattpocock + addyosmani）| ✅ 自动检查 + 安全应用 |
...
1. **读取** `skills/SOURCES.yaml`（19 个 skill，各含 source_repo + source_path）
...
- 已应用：2 个 verbatim 更新
- 复查：0 drift，14/14 verbatim 全部最新 ✅
...
```
ohMeisijiyaCode/
├── agents/                 # 7 个 agent prompt 文件
│   ├── onetwo.md         # primary (high-tier) — 7 XML segments + 12-row intent_gate
│   ├── twoone.md             # subagent (mid-tier) — 可委派给 EggDog
│   ├── eggdog.md       # subagent (low-tier) — task:deny, bash safe-glob
│   ├── architect.md        # subagent (high-tier) — 领域建模 + ADR 草案
│   ├── planner.md          # subagent (high-tier) — 实现计划 + to-issues
│   ├── reviewer.md         # subagent (high-tier) — 只读 code review
│   └── update.md           # subagent (high-tier) — CONTEXT.md/ADR/AGENTS.md 唯一写者
├── skills/                 # 19 个 skill（SKILL.md）
│   ├── caveman/                  # 来自 mattpocock（超压缩通信）
│   ├── diagnose/                 # 来自 mattpocock（6 阶段 bug 循环）
│   ├── git-workflow-and-versioning/ # 来自 addyosmani（git 工作流）
│   ├── grill-with-docs/          # 来自 mattpocock
│   ├── handoff/                  # 来自 mattpocock（跨 session 续接）
│   ├── improve-codebase-architecture/ # 来自 mattpocock（Architect 主用）
│   ├── incremental-implementation/   # 来自 addyosmani（纵向切片）
│   ├── interview-me/             # 来自 addyosmani（原样，一次一问）
│   ├── karpathy-guidelines/      # 4 大 karpathy 原则（meta-skill 自动加载）
│   ├── mmx-cli-usage/            # mmx CLI 指南（多模态）
│   ├── openspec-integration/     # OpenSpec ↔ Superpowers 路由桥
│   ├── prototype/                # 来自 mattpocock（一次性探索）
│   ├── setup-matt-pocock-skills/ # 来自 mattpocock（一次性脚手架）
│   ├── source-driven-development/ # 轻量复刻 addyosmani skill（用 ctx7 CLI）
│   ├── tdd/                      # 来自 mattpocock（红-绿-重构）
│   ├── to-issues/                # 来自 mattpocock（Planner 主用）
│   ├── triage/                   # 来自 mattpocock（5 角色 issue 状态机）
│   ├── update-project-meta/      # CONTEXT.md / ADR / AGENTS.md（update agent 用）
│   └── zoom-out/                 # 来自 mattpocock（高层视角）
├── .opencode/
│   └── commands/           # 21 个 slash-command 文件
...
│       │ prototype.md / setup.md / tdd.md / to-issues.md / triage.md /
...
├── VIBECODING.md           # 操作员手册 — 何时用什么 skill/command/agent
└── LICENSE                 # MIT
```
