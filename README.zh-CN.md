...
| 洞察 | 应用 |
|------|------|
| **U 型注意力曲线**（>50% context → 只关注末尾）| `<style_guide>` 是 3 agent prompt 的最后段；**末尾 HTML 注释块** 在 3 个 agent 都齐了（Sisyphus: 5 条 / Lyra: 4 / Hephaestus: 4），是 U 型注意力的"右锚点"|
...
| **反被动压缩**（不等质量下降再压缩）| **真实失败案例**：Sisyphus 363 行 prompt 中段 148 行协议被吞；用了 `task-dispatch` 工具而非 `task` 工具。修复：把协议下沉到 superpowers skill（实时注入），只把硬约束放在 U 型注意力尾部 |
