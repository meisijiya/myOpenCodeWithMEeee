...
|------|------|-------|-----------------|---------|
| **high** | 架构 / 规划 / 评审 / 元信息写入 | Sisyphus · Architect · Planner · Reviewer · Update | `anthropic/claude-opus-4-20250514` 或 `deepseek/deepseek-v4-pro` | ~$200 |
| **mid** | 复杂实现、调研、中等难度 bug | Lyra | `anthropic/claude-sonnet-4-20250514` 或 `deepseek/deepseek-v4-flash` | ~$20 |
| **low** | CRUD、机械重构、测试 boilerplate | Hephaestus | `anthropic/claude-haiku-4-20250514` 或 `deepseek/deepseek-v4-flash-free` | ~$0 |
...
安装后，opencode 默认提供两个 primary agent：**build** 和 **plan**。装了我们项目后，**Sisyphus**（也是 primary）变为可用入口，6 个 subagent（Lyra / Hephaestus / Architect / Planner / Reviewer / Update）通过 `@<agent>` 或 `task()` 触达。
...
| **build** | 只想跑单个 shell 命令/快速 read 文件，不需要 Sisyphus 的 7-agent 路由 |
| **plan** | 只做只读分析，零修改 |
...
也可以在对话里直接让 Sisyphus 委派（按它的 12 行 intent_gate 路由表自动选择——见下方[架构](#架构)）。
...
Sisyphus / Lyra 用 `task` 工具委派给 subagent。需要环境变量 `OPENCODE_EXPERIMENTAL=true`（或 `..._BACKGROUND_SUBAGENTS=true`）。
...
> 3. 内置 `task` 工具和 Sisyphus prompt 教的协议对齐——见 `agents/sisyphus.md` 的 `<delegation_protocol>` 段
...
| **反被动压缩**（不等质量下降再压缩）| **真实失败案例**：Sisyphus 363 行 prompt 变得难维护。v2.1 用了自研异步委派工具而非 opencode 内置 `task`——**不是因为"被 U 型注意力吞掉"**（协议在中段块中**是可见的**），而是**分层违规**（自研 wrapper 增加了维护成本 + 跟 opencode 升级 drift）。修复：v2.2 退役自研 wrapper，回到 opencode 内置 `task`——简化架构 > 短期"看起来更优雅"的自研 |
...
└──┬──────┬──────┬──────┬──────┬────────────────────────────┘
   ...
   ▼      ▼      ▼      ▼      ▼      ▼
┌────────┐┌────────┐┌────────┐┌────────┐┌─────────┐┌─────────┐
│Architect││Planner ││Reviewer││Update  ││Lyra     ││Hephaest.│
...
**Strict depth=3 rule**: Hephaestus + Update + Architect + Planner + Reviewer 全部 `task: false`（opencode 硬保证——Task 工具**被物理移除**）。Lyra + Sisyphus 可以委派子 agent，但 Lyra **禁止直接 spawn Hephaestus**——必须经过 Sisyphus intent_gate。无无限递归。

### Agent 角色速览

| Agent | Tier | 能做 | 不做 |
|-------|------|------|------|
| **Sisyphus** | high | 意图路由、OpenSpec 编排、skill 发现 | 实现、code review、元信息写入 |
...
| **Lyra** | mid | 复杂实现、调研、中等 bug | 架构设计、code review、写 plan |
| **Hephaestus** | low | CRUD、原子重构、测试脚手架 | 架构、规划、review、元信息写入 |
...
| DESIGN | 新 feature 设计（含单文件 + 设计内容）| Sisyphus（self） | high | yes |
| COMPLEX_CODE | 跨文件新 feature（≥2 文件需协调）| **Lyra** | mid | yes |
| RESEARCH | 调研、文档 | **Lyra** | mid | no |
| DEBUG_HARD | 复杂 bug（diagnose + fix + test verify）| **Lyra** | mid | no |
| DEBUG_SIMPLE | 明显 bug（单文件 ≤10 行）| Sisyphus（self） | high | no |
| CRUD | 重复写入（3+ 相似文件）| **Hephaestus** | low | no |
| ATOMIC_REFACTOR | 机械转换（跨文件重命名）| **Hephaestus** | low | no |
...
**边界情况**（避免"我自己干"的冲动）：
- "改 1 文件 + 跑 quick command" → DEBUG_SIMPLE / Sisyphus（self）
- "改 1 文件 + 跑完整 test suite" → DEBUG_HARD / Lyra（验证**就是**调研）
- "写 1 个设计文档（CONTRIBUTING.md）" → DESIGN / Sisyphus（self）
- "写 README + install.sh 联动" → COMPLEX_CODE / Lyra（多文件协调）
...
|-------|------|-----------|-----------|----------------------|
| **Sisyphus** | high | 主 agent（承载完整会话）| 架构决策 + 复杂推理 + **单文件 ≤10 行的小改** + 意图路由 | 跨多文件改动 / 复杂调研 / 简单 CRUD / 元信息写入 |
...
| 操作 | Sisyphus | Lyra | Hephaestus | Architect | Planner | Reviewer | Update |
|------|:--------:|:----:|:----------:|:---------:|:-------:|:--------:|:------:|
...
- **角色权限分层**：omo-slim 按角色给不同 agent 不同默认信任——只读 agent（`explorer`/`librarian`/`oracle`/`observer`）vs 读写 agent（`designer`/`fixer`）。我们也这样：Hephaestus 最宽松（worker 需要大量命令），Sisyphus/Lyra 中等（默认 allow + 硬 deny 黑名单），`build`/`plan` 最保守（opencode 出厂默认）。新加的 Architect/Planner/Reviewer/Update 按职责细化（Architect 仅写设计文档、Reviewer 只读、Update 仅写元信息文件、Planner 需要用户确认 bash）。
...
| **triage** | mattpocock/skills | 5 角色 issue 状态机 | Sisyphus / Planner |
| **improve-codebase-architecture** | mattpocock/skills | 发现"代码泥球"+ 建议架构改进 | Architect |
| **setup-matt-pocock-skills** | mattpocock/skills | 一次性脚手架（issue tracker + triage 标签）| Sisyphus（一次性）|
| **git-workflow-and-versioning** | addyosmani/agent-skills（原样导入）| Git 工作流：原子提交、分支策略、冲突解决 | 全部 |
| **incremental-implementation** | addyosmani/agent-skills（原样导入）| 纵向切片实现，补充 tdd | Lyra / Planner |
| **interview-me** | addyosmani/agent-skills（原样导入）| 需求不明确（缺 who/why/success/constraint）—— 一次问一个问题直到 95% 置信度 | Sisyphus / Planner |
...
| **tdd** | mattpocock/skills（原样导入）| 红-绿-重构循环的测试驱动开发 | Lyra / Hephaestus |
| **zoom-out** | mattpocock/skills（原样导入）| 放大视野，获取更广上下文和高层视角 | 全部（用户触发）|
| **update-project-meta** | 自研 | CONTEXT.md / ADR / AGENTS.md 元信息维护 | Update（单写者）/ Sisyphus |
...
| 跨文件实现 | 10% | 第 1 层（Lyra / Architect / Planner）|
...
| 模型 | 上下文 | 触发点 | 说明 |
|------|-------|--------|------|
| **MiniMax-M3**（high：Sisyphus / Architect / Planner / Reviewer / Update）| 512K | **~422K** | ✅ 512K 模型优化（reserved 50K + preserve 50K）|
| **deepseek-v4-flash**（mid/low：Lyra / Hephaestus）| 1M | ~910K | 激进 prompt 让 session 远离 910K |
...
**4. 下次 session 启动时**：
- Sisyphus 自动读取 `CONTEXT.md` / `AGENTS.md` / `docs/adr/`
- 项目上下文自动恢复——无需重新解释
...
│   ├── lyra.md             # subagent (mid-tier) — 可委派给 Hephaestus
